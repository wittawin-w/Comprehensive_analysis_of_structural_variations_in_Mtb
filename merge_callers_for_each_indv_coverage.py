...
for line in cat_file:
        ...
        line_cur = line.split("\t")
        if len(line_prev) > 0:
                lc = int(line_cur[5]) - int(line_cur[4]) + 1
                lp = int(line_prev[5]) - int(line_cur[4]) + 1
                if int(line_cur[4]) == int(line_prev[4]):
                        if int(line_cur[5]) > int(line_prev[5]) and float(lp/lc) < crit_cov:
                                define_var_group(var_group_list)
                                var_group_list = []
                elif int(line_cur[4]) > int(line_prev[4]):
                        if int(line_cur[5]) < int(line_prev[5]) and float(lc/lp) < crit_cov:
                                define_var_group(var_group_list)
                                var_group_list = []
                        elif int(line_cur[5]) == int(line_prev[5]) and float(lc/lp) < crit_cov:
                                define_var_group(var_group_list)
                                var_group_list = []
                        elif int(line_cur[5]) > int(line_prev[5]) and int(line_cur[4]) < int(line_prev[5]):
                                ov = int(int(line_prev[5]) - int(line_cur[4]) + 1)
