...
min_size=$5
max_size=$6 #Insertion does not use this parameter
bp=$7
...
while IFS='\t' read f1; do
        echo "sample name = $f1"
        sample_name=`echo $f1`
        for file in path/to/reformating/*_${sample_name}_addname.txt; do
                ls -lrt $file
        ...
        python3 path/to/merge_callers_for_each_indv_bp.py $output_dir/${sample_name}_all_callers_pos_sorted.txt $bp > $output_dir/${sample_name}_all_callers_merged.txt
        echo "Finish"
        sleep 1
done < $sample_list
rm -r $tmp_dir
